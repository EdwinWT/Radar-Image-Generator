"""
Navico Halo A Radar Image Generator
====================================
Decodes raw binary UDP spoke data captured from a Navico Halo A radar
via the OpenCPN radar_pi plugin and renders a PPI (Plan Position Indicator)
radar image.

Format reference: opencpn-radar-pi/radar_pi - NavicoReceive.cpp
  - Each UDP payload = one or more spokes
  - Each spoke = header + 512 nibble-packed return bytes (1024 range bins)
  - Angle: 2-byte LE at spoke offset 8, 0-4096 = 0-360 degrees
  - Range (Halo): br4g large/small fields at offsets 6-7 and 12-13 (see NavicoReceive.cpp)
  - Data: 512 bytes after 24-byte header, 2 x 4-bit nibbles per byte
  - Nibble values: 0x0-0xD = intensity, 0xE = receding Doppler, 0xF = approaching Doppler

Usage:
    python "Radar Image Generator.py" <binary_file> [--output radar.png] [--format auto]

Binary file formats supported:
    - auto:      Detect OpenCPN radar_raw BLOB, Navico frames, emulator, or length-prefixed UDP (default)
    - frame:     OpenCPN/SQLite radar_raw.data (8-byte frame_hdr + 32 spokes x 536 bytes)
    - emulator:  Emulator radar_raw.data (1440 spokes x 768 bytes, flat, no headers, 1 byte per bin)
    - raw:       UDP payloads with optional 2-byte LE length prefix per packet
    - pcap:      Wireshark capture (requires scapy)
    - img:       OpenCPN radar_img rgb BLOB (512x512 PNG output, not spokes)
"""
...
# OpenCPN emulator (EmulatorReceive.cpp)
# Flat spoke stream: no headers, no nibble packing, one uint8 per range bin.
EMULATOR_SPOKES     = 1440   # EMULATOR_SPOKES in emulatortype.h
EMULATOR_SPOKE_LEN  = 768    # EMULATOR_MAX_SPOKE_LEN in emulatortype.h
EMULATOR_ROTATION_BYTES = EMULATOR_SPOKES * EMULATOR_SPOKE_LEN  # 1,105,920 per full rotation
...
# ── Emulator sample reader ──────────────────────────────────────────────────────

def unpack_emulator_spoke(spoke_bytes: bytes) -> np.ndarray:
    """
    Emulator spokes are flat uint8 arrays (0-255), one byte per range bin --
    no nibble packing, no Doppler sentinels. Normalise to 0.0-1.0 here so the
    rest of the render pipeline treats them identically to Navico intensity.
    Returns a float32 array of length EMULATOR_SPOKE_LEN.
    """
    return np.frombuffer(spoke_bytes, dtype=np.uint8).astype(np.float32) / 255.0


def read_spokes_emulator(data: bytes) -> list:
    """
    Parse one or more concatenated emulator rotations from a radar_raw.data blob.

    Layout written by EmulatorReceive::FlushRotationBuffer():
        rotation_0: EMULATOR_SPOKES * EMULATOR_SPOKE_LEN bytes (flat, no headers)
        rotation_1: ...  (if multiple rows were exported and concatenated)

    Angle is not encoded in the bytes -- it is recovered positionally:
        spoke index N  =>  angle_deg = N * 360.0 / EMULATOR_SPOKES

    Range is not encoded either; pass range_m=0 and let the caller override via
    --range if needed for accurate range-ring labels.
    """
    n = len(data)
    if n == 0 or n % EMULATOR_SPOKE_LEN != 0:
        return []

    spokes = []
    n_spokes = n // EMULATOR_SPOKE_LEN
    for i in range(n_spokes):
        # Angle wraps every EMULATOR_SPOKES spokes so multi-rotation blobs work.
        angle_index = i % EMULATOR_SPOKES
        angle_deg = angle_index * 360.0 / EMULATOR_SPOKES
        start = i * EMULATOR_SPOKE_LEN
        spoke_bytes = data[start:start + EMULATOR_SPOKE_LEN]
        spokes.append((angle_deg, 0.0, spoke_bytes))

    return spokes


# ── Emulator-aware PPI renderer ─────────────────────────────────────────────────

def render_ppi_emulator(spokes, image_size=1024):
    """
    Vectorised PPI renderer for emulator spokes.

    Differences from the Navico renderer:
      - Intensity is already 0.0-1.0 float (full uint8 scale, not nibble 0-13).
      - No Doppler channel (emulator has no 0xE/0xF sentinels).
      - Spoke length is EMULATOR_SPOKE_LEN (768), not SAMPLES_PER_SPOKE (1024).
    """
    cx = cy = image_size // 2
    ...
    bins   = np.arange(EMULATOR_SPOKE_LEN, dtype=np.float32)
    r_frac = (bins + 0.5) / EMULATOR_SPOKE_LEN

    for angle_deg, _range_m, spoke_bytes in spokes:
        samples   = unpack_emulator_spoke(spoke_bytes)   # float32, 0-1
        ...
        np.maximum.at(intensity, (pys[mask], pxs[mask]), samples[mask])

    # Emulator has no Doppler -- return empty boolean arrays so render_image()
    # works unchanged (it checks .any() before drawing the overlays).
...
def detect_data_format(data: bytes) -> str:
    ...
    if n == 512 * 512 * 3:
        return "img"
    # Emulator: flat spoke stream, no headers.
    # Must be an exact multiple of EMULATOR_SPOKE_LEN (768) and NOT a valid
    # Navico frame (already ruled out above), and large enough to be at least
    # one spoke.  We also check that the data doesn't look like a Navico header
    # at offset 0 (first byte == 0x18, second in {0x02, 0x12}).
    if n >= EMULATOR_SPOKE_LEN and n % EMULATOR_SPOKE_LEN == 0:
        if data[0] != NAVICO_HEADER_LEN or data[1] not in (0x02, 0x12):
            return "emulator"
    return "unknown"
...
def read_spokes_raw(path: Path, fmt: str = "auto"):
    """
    Read radar spokes from a binary file.

    fmt: auto | frame | emulator | raw | length_prefixed
    """
    ...
    if fmt == "emulator":
        return read_spokes_emulator(data)
...
def read_radar_img_rgb(path: Path):
    ...
    data = path.read_bytes()
    for header_len in (0, 8, 16, 18, 24, 32, 64):
...
def render_image(intensity, dop_app, dop_rec, range_m, output_path, image_size=1024):
...
def main():
    parser = argparse.ArgumentParser(
        description="Decode Navico Halo A or OpenCPN emulator raw radar data and produce a PPI image."
    )
    parser.add_argument("input", help="Path to binary radar data file")
    parser.add_argument("--output", default=None,
                        help="Output image path (default: same as input file)")
    parser.add_argument(
        "--format",
        choices=["auto", "frame", "emulator", "raw", "pcap", "img"],
        default="auto",
    ...
    parser.add_argument("--range", type=float, default=None, dest="range_m",
                        help="Radar range in metres (for emulator data, which does not encode range). "
                             "Used only for range-ring labels.")
    args = parser.parse_args()

    path = Path(args.input)
    if args.output is None:
        args.output = path.with_suffix('.png') 
    if not path.exists():
    ...
    # Detect whether these are emulator or Navico spokes by checking if any
    # spoke carries non-zero range (emulator always stores 0.0).
    is_emulator = all(r == 0.0 for _, r, _ in spokes)

    # Range for ring labels: --range flag overrides, otherwise read from spokes
    # (Navico only; emulator doesn't encode range in the byte stream).
    ...
    if is_emulator:
        print(f"Parsed {len(spokes)} emulator spokes "
              + (f"(range override: {range_m/1852:.2f} nm)" if range_m else "(no range -- use --range to label rings)"))
    else:
        print(f"Parsed {len(spokes)} spokes, range ~{range_m/1852:.2f} nm")

    print("Rendering PPI image ...")
    if is_emulator:
        intensity, dop_app, dop_rec = render_ppi_emulator(spokes, args.size)
    elif args.fast:
        intensity, dop_app, dop_rec = render_ppi_fast(spokes, args.size)
    else:
        intensity, dop_app, dop_rec = render_ppi(spokes, args.size)

    from scipy.ndimage import grey_dilation
    intensity = grey_dilation(intensity, size=(2, 2))
